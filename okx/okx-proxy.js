...
class OKX {
    ...
    async postToOKXAPI(extUserId, extUserName, proxy) {
    ...
    async assessPrediction(extUserId, predict, proxy) {
    ...
    async checkDailyRewards(extUserId, proxy) {
        ...
        try {
            ...
            if (dailyCheckInTask) {
                if (dailyCheckInTask.state === 0) {
                    ...
                    await this.performCheckIn(extUserId, dailyCheckInTask.id, proxy);
                } else {
    ...
    async performCheckIn(extUserId, taskId, proxy) {
        ...
        while (true) {
            for (let i = 0; i < userData.length; i++) {
                ...
                try {
                    ...
                    await this.checkDailyRewards(extUserId, proxy);
                    for (let j = 0; j < 50; j++) {
                        const response = await this.postToOKXAPI(extUserId, extUserName, proxy);
                        const balancePoints = response.data.data.balancePoints;
                        this.log(`${'Balance Points:'.green} ${balancePoints}`);

                        const predict = 1;
                        const assessResponse = await this.assessPrediction(extUserId, predict, proxy);
